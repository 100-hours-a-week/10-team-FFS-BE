...
import com.example.kloset_lab.ai.infrastructure.http.client.AIClient;
...
public class AiService {
     ...
     * TPO 코디 생성 요청
     *
     * <p>AI-BE 호출(수십 초) 동안 DB 커넥션을 점유하지 않도록 트랜잭션 없이 실행한다. MediaFile
     * 생성(TX1)은 aIClient 내부에서, TpoRequest/TpoResult 저장 및 MediaFile 확정(TX2)은
     * TpoSaveService에서 별도 트랜잭션으로 처리한다.
     *
    ...
    public TpoOutfitsResponse generateTpoOutfits(Long userId, @Valid TpoOutfitsRequest request) {
        ...
        OutfitResponse outfitResponse = aIClient.recommendOutfit(userId, request.content());

        return tpoSaveService.saveAndBuild(userId, request.content(), outfitResponse);
    }
